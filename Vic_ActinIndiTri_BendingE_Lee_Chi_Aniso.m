%%%% Calculate the bending energy, Lee and others of actin filaments with 
%%%% isolated triangular pillar and save.
%
% data from Vic_ActinIndiTri_AddPAsInfo.m (AddInfo_trajectory_..._batch1.mat)
...
for file_ind = 1:length(Files)
    ...
    if contains(filename, 'AddInfo_')
        ...
        [intensity_base, ~, ~] = Vic_Fiber_Intensity(xy, the_fully_stretched_No, base_fiber_no, base_II, 30);

        CoM_x = zeros(size(Good_case_frm,2), 1); CoM_y = zeros(size(Good_case_frm,2), 1);
        ...
        for frm_ind = 1:size(Good_case_frm,2)
            ...
            [~, IntensityAll,R2] = Vic_Fiber_Intensity(xy, frm_ind, xy_ind, II, 30);
        ...
        save([save_pathname, filesep, 'PlusInfo_', filename(14: end-4), '.mat'], ...
...
function [intensity_0, IntensityAll, R] = Vic_Fiber_Intensity(xy, ~, xy_ind, II, cut_length)
...
spl(:, 2) = size(II, 2) - spl(:, 2); % convert the x-y coordinates to 'image' coordinate
...
for seg_ind = 1:size(spl,1)
    ...
    if ~isnan(vec_0(1))
        ...
        imshow(II, []);hold on
        plot(xy.spl{xy_ind}(:,1),size(II, 2)-xy.spl{xy_ind}(:,2), 'LineWidth', 2);hold on
