...
[excelname, excelpathname] = uigetfile(['D:\Dropbox\Collaboration - LadHyX\' ...
    'give_to_Zhibo\.xlsx'], 'Please choose the excel accordingly and carefully!!');
...
obs = readVTK('D:\Dropbox\Collaboration - LadHyX\give_to_Zhibo\obstacle_beads.vtk');
...
parent_path = 'D:\Dropbox\Collaboration - LadHyX\give_to_Zhibo\data';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% plot %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
xlsfile = readcell(['D:\Dropbox\Collaboration - LadHyX\give_to_Zhibo\' ...
    'results-dynamics.xlsx'],'Sheet','Sheet1','NumHeaderLines',1); 
together_plot = [cell2mat(xlsfile(:, 1:4)), cell2mat(xlsfile(:, 7:8)), cell2mat(xlsfile(:, 11:14))]; 
...
% y_0
together_plot(:, together_plot(3, :) < 0) = [];  % the initial position: 0 < y_0 < 1
together_plot(:, together_plot(3, :) > 1) = [];


figure('color', 'w'); set(gcf, 'Position', [100 100 1200 600]);
% cmap = cmocean('thermal');
cmap = colormap("jet");
bypass_edge_together = together_plot(:, ~logical(together_plot(7, :))); 
bypass_tip_together = together_plot(:, logical(together_plot(7, :)));
pole_vaulting_together = together_plot(:, logical(together_plot(5, :))); 
trapped_together = together_plot(:, logical(together_plot(6, :)));
...
scatter(trapped_together(10, :), trapped_together(4, :), 220, trapped_together(3, :), 'Filled', 'diamond'); hold on 
scatter(bypass_edge_together(10, :), bypass_edge_together(4, :), 200, bypass_edge_together(3, :), 'Filled'); hold on
scatter(bypass_tip_together(10, :), bypass_tip_together(4, :), 200, bypass_tip_together(3, :), 'Filled', 'square'); hold on
scatter(pole_vaulting_together(10, :), pole_vaulting_together(4, :), 220, pole_vaulting_together(3, :), 'Filled', '^'); hold on
...
% f=gcf;
% exportgraphics(f,'y0_vs_interaction3-delta_classification_simulationdata_lessDyn.png','Resolution',100)
